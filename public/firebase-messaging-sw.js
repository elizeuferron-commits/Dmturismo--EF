...
const CACHE_NAME = 'dm-turismo-static-v3';
const EXTERNAL_CACHE_NAME = 'dm-turismo-external-v3';
    ...
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames.map((cache) => {
          if (cache !== CACHE_NAME && cache !== EXTERNAL_CACHE_NAME) {
...
self.addEventListener('fetch', (event) => {
  ...
  // Refined stale-while-revalidate strategy specifically for Unsplash images and fonts (gstatic, googleapis)
  const isFontOrUnsplash =
      ...
      caches.open(EXTERNAL_CACHE_NAME).then((cache) => {
        return cache.match(event.request).then((cachedResponse) => {
          const fetchPromise = fetch(event.request).then((networkResponse) => {
            // Cache valid 200 requests as well as opaque (status 0) cross-origin requests
            if (networkResponse && (networkResponse.status === 200 || networkResponse.status === 0)) {
              cache.put(event.request, networkResponse.clone());
            }
            return networkResponse;
          }).catch((err) => {
            console.warn('[SW] Font/Unsplash fetch failed:', err);
          });
  ...
  // Dedicated check for DM logo and icons/manifest with Cache-Control headers
  ...
  if (isLogoOrPwaIcon) {
    event.respondWith(
      caches.match(event.request).then((cachedResponse) => {
        ...
        if (cachedResponse) {
          // Serve from cache with injected Cache-Control header
          return injectCacheControl(cachedResponse);
        }

        // Cache miss: fetch from network, inject Cache-Control, cache it, and return
        return fetch(event.request).then((networkResponse) => {
          ...
          caches.open(CACHE_NAME).then((cache) => {
            injectCacheControl(responseToCache).then((modifiedResponse) => {
          ...
          return injectCacheControl(networkResponse);
        }).catch(() => {
          // Offline fallback
        });
      })
  ...
  // Network-First strategy for main document / navigation requests to prevent stale index.html stuck screens
  ...
  // Intercept requests for images, icons, and bundler assets
...
try {
  ...
  console.warn('[SW] Firebase Messaging could not be initialized:', e);
}
